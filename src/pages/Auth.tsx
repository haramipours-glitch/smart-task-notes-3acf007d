...
export default function Auth() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      if (isLovableHost()) {
        // Lovable preview/hosting uses its own OAuth broker.
      ...
      // Self-hosted / custom domains: use native Supabase OAuth.
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: { redirectTo: absoluteReturnTo, skipBrowserRedirect: true },
      });
      ...
      throw new Error("هیچ آدرس OAuth دریافت نشد");
    } catch (err) {
