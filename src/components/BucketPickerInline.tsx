...
export function BucketPickerInline({
  value,
  onChange,
}: {
  value: { kind: BucketKind | null; calendar: CalendarSystem | null; anchor: string | null };
  onChange: (v: { kind: BucketKind | null; calendar: CalendarSystem | null; anchor: string | null }) => void;
}) {
  const enabled = getEnabledBuckets();
  const cal = getCalendarSystem();
  const current = value.kind && value.anchor && value.calendar
    ? `${kindLabel(value.kind)} · ${bucketLabel(value.kind, value.calendar, value.anchor)}`
    : "انتخاب بازه";
    ...
    <Popover>
      ...
      <PopoverContent className="w-64 p-1" align="end">
        ...
        {ALL_BUCKET_KINDS.filter((k) => enabled.includes(k)).map((k) => {
          ...
          const active = value.kind === k && value.anchor === anchor && value.calendar === cal;
          return (
            <button
              key={k}
